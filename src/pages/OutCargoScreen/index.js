...
export default class index extends React.Component {
    ...
    onSave = async () => {
        try {
            ...
            asyncStorage.getItem('chuKu', async (error, result) => {
                if (!error) {
                    if (result) {
                        let arrList = [];
                        ...
                        await this.setStorageData([...arrList, ...dataList], "chuKu");

                    } else {
                        await this.setStorageData(dataList, "chuKu");
                    }
                }
            })
        } catch (e) {
    ...
    getStorageData = async () => {
        const chuKuString = await asyncStorage.getItem("chuKu");
        if (chuKuString) {
            let arrList = [];
            const arrData = JSON.parse(chuKuString);
            if (arrData.length) {
                for (let i = 0; i < arrData.length; i++) {
                    const list = await asyncStorage.getItem(arrData[i]);
                    arrList = [...arrList, ...JSON.parse(list)];
